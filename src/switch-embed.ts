/**
 * Embeddable Switch Component
 * A wrapper that allows the Jelly Switch to be initialized on a specific canvas element
 */
...
export async function initSwitch(canvas: HTMLCanvasElement, callbacks?: SwitchCallbacks) {
    ...
    let qualityScale = 0.7; // Start with medium quality for embedded view
    let [width, height] = [
    ...
    const jellyColorUniform = root.createUniform(
        d.vec4f,
        d.vec4f(0.08, 0.5, 1.0, 1.0), // Blue color for switch
    );

    const darkModeUniform = root.createUniform(d.u32, d.u32(1)); // Start in dark mode
    const randomUniform = root.createUniform(d.vec2f);

    // GPU functions (same as original)
    ...
    const getJellyBounds = () => {
        'use gpu';
        return BoundingBox({ min: d.vec3f(-1, -1, -1), max: d.vec3f(1, 1, 1) });
    };
    ...
    const renderBackground = (rayOrigin: d.v3f, rayDirection: d.v3f, backgroundHitDist: number) => {
        ...
        const backgroundColor = applyAO(
            ...
            hitPosition,
            newNormal,
        ).add(d.vec4f(bounceLight.mul(emission), 0)).add(d.vec4f(sideBounceLight.mul(emission), 0));
        ...
        for (let i = 0; i < MAX_STEPS; i++) {
            ...
            if (hitInfo.distance < SURF_DIST) {
                ...
                if (k > 0.0) {
                    ...
                    const state = switchBehavior.stateUniform.$;
                    const progress = std.saturate(std.mix(1, 0.6, hitPosition.y * (1 / (JELLY_HALFSIZE.y * 2)) + 0.25)) * state.progress;
    ...
    canvas.addEventListener('touchend', () => {
        ...
        callbacks?.onToggle?.(currentToggleState);
    });
    ...
    return {
        ...
        isToggled: () => currentToggleState,
        setDarkMode: (dark: boolean) => darkModeUniform.write(d.u32(dark ? 1 : 0)),
        setColor: (r: number, g: number, b: number) => jellyColorUniform.write(d.vec4f(r, g, b, 1.0)),
        destroy: () => {
            isRunning = false;
